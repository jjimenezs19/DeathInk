<div class="container " style="background-color:#e8e8e8; ">
    <div class="container_inner">
        <div class="row_grid" style="padding-left: 15px; padding-right: 15px;">
            <div class="columns-3 posts">
                <div class="h-column">
                    <article id="1">
                        <div class="post_image" style="background-image:url(img/post2.jpg);"><a href="/tattoo-care"></a></div>
                        <div class="post_content">
                            <p class="post_name">Post</p>
                            <a href="/tattoo-care"><h4>Tattoo Care</h4></a>
                            <p class="post_date">04.04.2017</p>
                            <p class="post_text">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed totam rem aperiam, eaque ipsa quae ab illo inventore. Lorem ipsum dolor sit amet.</p>
                            <a class="more_gallery" href="/tattoo-care">Read more</a>
                            <div class="clearfix"></div>
                        </div>
                    </article>
                </div>

                <div class="h-column">
                    <article id="2">
                        <div class="post_image" style="background-image:url(img/post1.jpg);"><a href="#"></a></div>
                        <div class="post_content">
                            <p class="post_name">Post</p>
                            <a href="#"><h4>Lorem ipsum lorem lorem ipsum</h4></a>
                            <p class="post_date">04.04.2017</p>
                            <p class="post_text">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed totam rem aperiam, eaque ipsa quae ab illo inventore. Lorem ipsum dolor sit amet.</p>
                            <a class="more_gallery" href="#">Read more</a>
                            <div class="clearfix"></div>
                        </div>
                    </article>
                </div>

                <div class="h-column">
                    <article id="3">
                        <div class="post_image" style="background-image:url(img/post3.jpg);"><a href="#"></a></div>
                        <div class="post_content">
                            <p class="post_name">Post</p>
                            <a href="#"><h4>Lorem ipsum lorem lorem ipsum</h4></a>
                            <p class="post_date">04.04.2017</p>
                            <p class="post_text">Lorem ipsum dolor sit amet, consectetur adipisicing elit, sed totam rem aperiam, eaque ipsa quae ab illo inventore. Lorem ipsum dolor sit amet.</p>
                            <a class="more_gallery" href="#">Read more</a>
                            <div class="clearfix"></div>
                        </div>
                    </article>
                </div>
            </div>
        </div>
    </div>
    <div class="clearfix"></div>
</div>